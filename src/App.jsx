...
function AnimatedBackground({ scrollY }) {
  ...
  useEffect(() => {
    ...
    resize()
    window.addEventListener('resize', resize)
    const particles = Array.from({ length: 150 }, () => ({ x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, vx: (Math.random() - 0.5) * 0.4, vy: (Math.random() - 0.5) * 0.4, size: Math.random() * 1.5 + 0.3, opacity: Math.random() * 0.6 + 0.1 }))
  ...
  return (
    <div className="min-h-screen bg-[#030712]">
      ...
      <div className="content-layer">
        <nav className="px-8 py-5 flex items-center justify-between sticky top-0 z-50 backdrop-blur-md border-b border-white/5">
        ...
        {resumeText ? (
          ...
          <>
            <section className="min-h-screen flex flex-col items-center justify-center px-4 text-center gap-8 relative">
              ...
              <div className="flex flex-col gap-1">
                <h1 className="text-7xl font-black text-white leading-none tracking-tighter uppercase">ACE YOUR</h1>
                <h1 className="text-7xl font-black leading-none tracking-tighter uppercase text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-cyan-400 to-blue-600">INTERVIEW</h1>
                <h1 className="text-7xl font-black text-white/20 leading-none tracking-tighter uppercase">BEFORE IT</h1>
                <h1 className="text-7xl font-black text-white/20 leading-none tracking-tighter uppercase">HAPPENS</h1>
              </div>
              <p className="text-white/40 text-base leading-relaxed max-w-md">Upload your resume. Your personal AI hiring manager reads it and conducts a real voice interview — questions built around your actual experience.</p>
              <div className="flex items-center gap-4">
                <button onClick={() => setShowUpload(true)} className="group bg-blue-600 hover:bg-blue-500 text-white px-10 py-4 rounded-full font-bold text-sm uppercase tracking-widest transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/30">Start Interview <span className="ml-2 group-hover:translate-x-1 inline-block transition-transform">→</span></button>
              ...
              <div className="flex items-center gap-12 mt-4">
                {[{ num: '5', label: 'Questions' }, { num: 'AI', label: 'Voice Interview' }, { num: '∞', label: 'Practice Rounds' }].map((s, i) => (
                  <div key={i} className="text-center"><p className="text-3xl font-black text-white">{s.num}</p><p className="text-white/30 text-xs uppercase tracking-widest mt-1">{s.label}</p></div>
                ))}
              </div>
            ...
            <section className="py-32 px-8 max-w-5xl mx-auto">
              <Reveal>
                <div className="text-center mb-16"><p className="text-indigo-400 text-xs uppercase tracking-widest mb-3">The Process</p><h2 className="text-5xl font-black text-white uppercase tracking-tight">How It Works</h2></div>
              </Reveal>
              <Reveal delay={1}>
                <div className="grid grid-cols-3 gap-6">
                  {[{ step: '01', icon: '📄', title: 'Upload Resume', desc: 'Drop your PDF resume into the app. Our AI reads every line — your experience, skills, and projects.' }, { step: '02', icon: '🤖', title: 'AI Reads It', desc: 'The hiring manager AI analyzes your background and builds personalized questions just for you.' }, { step: '03', icon: '🎤', title: 'Voice Interview', desc: 'Speak your answers out loud like a real interview. Get instant feedback after every question.' }].map((s, i) => (
                    <div key={i} className="relative bg-white/5 border border-white/10 rounded-2xl p-8 hover:bg-white/8 hover:border-indigo-500/30 transition-all duration-500 group">
                      <div className="text-6xl font-black text-white/5 absolute top-6 right-6 group-hover:text-indigo-500/10 transition-all">{s.step}</div>
            ...
            <section className="py-32 px-8 max-w-5xl mx-auto">
              <Reveal>
                <div className="text-center mb-16"><p className="text-purple-400 text-xs uppercase tracking-widest mb-3">What You Get</p><h2 className="text-5xl font-black text-white uppercase tracking-tight">Features</h2></div>
              </Reveal>
              <Reveal delay={1}>
            ...
            <section className="py-32 px-8 max-w-5xl mx-auto">
              <Reveal>
                <div className="text-center mb-16"><p className="text-cyan-400 text-xs uppercase tracking-widest mb-3">Who It's For</p><h2 className="text-5xl font-black text-white uppercase tracking-tight">Built For You</h2></div>
              </Reveal>
              <Reveal delay={1}>
                <div className="grid grid-cols-3 gap-4">
                  {[{ icon: '🎓', title: 'First Job Hunters', desc: 'Landing your first job is nerve-wracking. Practice until the interview feels natural.' }, { icon: '🔄', title: 'Career Switchers', desc: 'Pivoting industries? Practice framing your past experience for a new role.' }, { icon: '📈', title: 'Promotion Seekers', desc: 'Going for a senior role? Practice articulating your impact and leadership.' }].map((w, i) => (
                    <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center hover:bg-white/8 hover:border-cyan-500/20 transition-all duration-300">
            ...
            <section className="pt-24 pb-32 px-8 text-center">
              <Reveal>
                <div className="max-w-2xl mx-auto flex flex-col items-center gap-8">
                  <div className="flex flex-col items-center gap-2">
                    <h2 className="text-7xl font-black text-white uppercase tracking-tight leading-none">Ready to</h2>
                    <h2 className="text-7xl font-black uppercase tracking-tight leading-none text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Practice?</h2>
                  </div>
                  <p className="text-white/40 text-sm">Free. No signup. Just upload your resume and go.</p>
                  <button onClick={() => setShowUpload(true)} className="group bg-blue-600 hover:bg-blue-500 text-white px-12 py-5 rounded-full font-bold text-sm uppercase tracking-widest transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/40">Start Interview <span className="ml-2 group-hover:translate-x-1 inline-block transition-transform">→</span></button>
                </div>
              </Reveal>
            </section>
            <footer className="border-t border-white/5 px-8 py-10">
              <div className="max-w-5xl mx-auto flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-2"><div className="w-6 h-6 bg-blue-500/20 border border-blue-500/30 rounded-md flex items-center justify-center text-xs">🎤</div><span className="text-white font-bold text-sm tracking-wider uppercase">HirePrep AI</span></div>
          ...
          <div className="min-h-screen flex items-center justify-center px-4">
            <div className="w-full max-w-lg flex flex-col items-center gap-6 animate-fade-in-up">
              ...
              <div className="text-center"><h2 className="text-4xl font-black text-white uppercase tracking-tight">Upload Resume</h2><p className="text-white/40 text-sm mt-2">We'll read it and build your interview</p></div>
